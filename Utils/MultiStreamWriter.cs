...
namespace AudioConsolidator.Utils;
...
public class MultiStreamWriter : TextWriter
{
    ...
    public MultiStreamWriter AddWriter(params Stream[] writer) {
        foreach (var stream in writer)
            writers.Add(new StreamWriter(stream));
    ...
    /// <returns></returns>
    public MultiStreamWriter SetEncoding(Encoding value) {
        encoding = value;
        return this;
    }
    ...
    public override IFormatProvider FormatProvider => formatProvider ?? base.FormatProvider;
    ...
    /// <param name="value">The value.</param>
    /// <returns></returns>
    MultiStreamWriter SetFormatProvider(IFormatProvider value) {
